...
```
查询流程：
1️⃣  Backend API (如果启用)
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 继续
2️⃣  OpenBB (本地 yfinance)
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 报错
```

**特点**：
- 后端 API 通过 Yahoo Finance 获取美股数据
- OpenBB 是兜底方案，本地离线也能工作
- 无缓存层（实时数据）

---

### 港股（HK Stock）
例如：`0700.HK`（腾讯）、`9988.HK`（阿里）、`2318.HK`（平安）
...
### A 股（CN Stock）
例如：`600519.SS`（茅台）、`000858.SZ`（五粮液）、`601318.SS`（中国平安）

```
查询流程（Quote 命令）：
1️⃣  Backend MCP (如果启用) ⭐ 优先级最高
    ├─ Tushare Pro 数据源（最高质量）
    ├─ 五层缓存策略
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 继续
2️⃣  Backend API (如果启用)
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 继续
3️⃣  arti-data 专属服务 (如果配置)
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 继续
4️⃣  OpenBB (本地兜底)
    ├─ ✓ 成功 → 返回数据
...
查询流程（Technical 指标）：
1️⃣  Backend MCP (如果启用) ⭐ 推荐用 MCP
    ├─ MA/RSI/MACD/Bollinger/ATR 等
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 继续
2️⃣  Backend API (如果启用)
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 继续
3️⃣  arti-data (基于 K 线数据计算)
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 继续
4️⃣  OpenBB (基于本地 K 线计算)
    ├─ ✓ 成功 → 返回数据
    └─ ✗ 失败 → 报错
```
...
**特点**：
- **格式**：
  - 上海证交所：`XXXXXX.SS`（6 位代码 + .SS）
  - 深圳证交所：`XXXXXX.SZ`（6 位代码 + .SZ）
- **最高优先级是 MCP**：Tushare Pro 数据质量最好，缓存策略最完善
- **A 股独占**：arti-data 和 MCP 都只提供 A 股数据
- **符号标准化**：支持 `600519` / `600519.SS` 两种格式
