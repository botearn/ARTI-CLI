...
- `src/core/conversation-session.ts` — 0700/0600 JSONL Session、Artifact、索引、恢复和保留期清理
...
`/compact [focus]` 第一版只手动触发，调用普通 `v1-chat` 并按聊天规则计费；它只写入 summary boundary，不删除 transcript。Slash 能力的完整结构化结果写入当前 Session 的 Artifact，活跃上下文只传 digest；Artifact 不跨 Session，并随 Session 保留期清理。外层 `arti <command> --json` 不创建本地 Session 或 Artifact。
