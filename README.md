...
| 命令 | 说明 |
|---|---|
| `arti quote AAPL NVDA` | 实时行情（支持多股、港股、中文名搜索） |
...
| `arti search 苹果` | 搜索股票代码（模糊匹配） |
| `arti news AAPL` | 公司新闻 |
| `arti news` | 全球财经新闻 |
| `arti research AAPL` | AI 三层级研报（8 位分析师 → 大师辩论 → 综合裁定） |
| `arti watchlist` | 查看自选股行情 |
| `arti watchlist add AAPL` | 添加 / 移除自选股 |
| `arti watch AAPL NVDA` | 实时行情 Dashboard（自动轮询，Ctrl+C 退出） |
| `arti export AAPL -f csv` | 导出历史数据到 CSV / JSON 文件 |
| `arti insights` | 个人投研洞察报告（HTML 可分享） |
| `arti completion zsh` | 生成 Shell 自动补全脚本 |
| `arti config list` | 查看 / 修改配置 |
